...
def _get_int_blood_code(blood_type):
    
    if type(blood_type) == int:
...
def check_bt(donor, recipient):
    ...
    """
    donor_numeric_code = _get_int_blood_code(donor)
    recipient_numeric_code = _get_int_blood_code(recipient)
    
    if donor_numeric_code == -1 or recipient_numeric_code == -1:
        return False
    
    compatibility = _particular_antigen_comp(
        donor_numeric_code, 
        recipient_numeric_code
    )

    print(compatibility)
    print([value < 0 for value in compatibility])
    
    return not any([value < 0 for value in compatibility])
    
    
    
    # Here I have to have an integer
