...
def get_first_model_each_manufacturer(cars: CarsType = cars) -> List[str]:
    ...
    return [cars[manufacturer][0] for manufacturer in cars]
...
def get_all_matching_models(
    ...
    """
    # Use the filter idea to keep only elements with grep inside
    filtered_models = [list(filter(lambda model: grep.lower() in model.lower(), models)) for models in cars.values()]

    # Here, use chain to transform a list of lists into a unique one.
    return sorted(list(chain(*filtered_models)))
...
def sort_car_models(cars: CarsType = cars) -> CarsType:
    ...
    return {
        manufacturer: sorted(cars[manufacturer])
